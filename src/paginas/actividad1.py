...
class estado(pantalla.Pantalla):
    ...
    def __init__(self, parent):
        ...
        self.insec = PropObject(
            760, 140, self.varios + "insec.png", "el controlador biológico. "
        )
        self.regadera = PropObject(
            792, 270, self.varios + "regadera.png", "la regadera. "
        )
        self.semillas = PropObject(
            450, 200, self.varios + "semillas.png", "las semillas. "
        )
        self.nubes = animacion(
            "nubes", self.varios + "nubes.png", 1, 1, 30, -15, -1, False, 18
        )
        self.salir = Button(
            830, 60, "salir", "Salir", self.botones + "boton-salir.png", 1, -1, False, 1
        )
        self.flecha = animacion(
            "flecha", self.varios + "flecha-verde.png", 3, 1, 800, 350, -1, True, 6
        )
        self.flores = animacion(
            "flores", self.varios + "campo-flores.png", 4, 1, 758, 290, -1, False, 18
        )
        self.siembra = animacion(
            "siembra", self.varios + "cinta-campesino.png", 3, 1, 680, 250, -1, True, 9
        )
        self.granjero = personaje(200, 128, self.varios + "0.png", 2)
        self.popup_respuesta = PopUp(
            parent, ("Respuesta "), "Aceptar", self.flecha_verde, self.grupo_popup
        )
    ...
    def nivel1(self):
        ...
        self.limites.empty()
        self.limites.add(
            limite((167, 267, 170, 20), 1),
            limite((317, 196, 20, 91), 2),
            limite((317, 196, 113, 20), 3),
            limite((410, 196, 20, 91), 4),
            limite((410, 267, 430, 20), 5),
            limite((167, 267, 20, 117), 6),
            limite((820, 267, 20, 117), 7),
            limite((167, 364, 310, 20), 8),
            limite((550, 364, 290, 20), 9),
            limite((457, 364, 20, 208), 10),
            limite((550, 364, 20, 122), 11),
            limite((550, 466, 169, 20), 12),
            limite((699, 466, 20, 106), 13),
            limite((457, 552, 262, 20), 14),
        )
    ...
    def nivel2(self):
        ...
        self.limites.empty()
        self.limites.add(
            limite((153, 124, 20, 259), 1),
            limite((153, 124, 113, 20), 2),
            limite((246, 124, 20, 92), 3),
            limite((246, 196, 151, 20), 4),
            limite((377, 124, 20, 92), 5),
            limite((377, 124, 113, 20), 6),
            limite((470, 124, 20, 92), 7),
            limite((470, 196, 148, 20), 8),
            limite((598, 124, 20, 92), 9),
            limite((598, 124, 117, 20), 10),
            limite((695, 124, 20, 92), 11),
            limite((695, 196, 177, 20), 12),
            limite((852, 196, 20, 187), 13),
            limite((760, 363, 112, 20), 13),
            limite((760, 293, 20, 90), 14),
            limite((555, 293, 225, 20), 15),
            limite((555, 293, 20, 120), 16),
            limite((555, 393, 170, 20), 17),
            limite((705, 393, 20, 118), 18),
            limite((463, 491, 262, 20), 19),
            limite((463, 293, 20, 218), 20),
            limite((246, 293, 237, 20), 21),
            limite((246, 293, 20, 90), 22),
            limite((153, 363, 113, 20), 23),
        )
        self.granjero.por_defecto(150, 50, self.varios + "-1.png", 2, self.limites)
        self.granjero.actualizar_rects()
        self.granjero.codigo = -1
        self.grupo_personaje.add(self.granjero)
        self.grupo_objetos.add(
            self.carre, self.pala, self.regadera, self.semillas, self.abono, self.insec
        )
        ...
        for evento in eventos:
            ...
            if self.popup_respuesta.activo and not self.popup_ayuda.activo:
                ...
                if (
                    ...
                    or self.popup_respuesta.evaluar_click() != -1
                ):
                    if self.respuesta == 1:
                        self.actualizar_pj()
            ...
            if self.popup_final1.activo:
                ...
                if (
                    self.teclasPulsadas[pygame.K_RETURN]
                    or self.popup_final1.evaluar_click() != -1
                ):
        ...
        if self.inicio_cont and not self.final_cont:
            if self.nivel_actual == 1:
                ...
                if self.tiempo > 13000:
                    ...
                    self.final_cont = True
                    self.granjero.ocupado = False
                    self.grupo_marcadores.add(
                        self.m_sem_car, self.m_rega, self.m_abono, self.m_insec
                    )
                else:
                    self.tiempo += self.contador.get_time()
    ...
    def logica(self):
        ...
        """
        if (self.granjero.codigo == 7 and self.nivel_actual == 1) or (
            self.nivel_actual == 2 and self.granjero.codigo >= 31
        ):
            if not self.completado:
        ...
        if (
            self.meta.colliderect(self.granjero.rt_car)
            and self.granjero.codigo == 7
            and self.nivel_actual == 1
        ):
        ...
        if (
            self.meta.colliderect(self.granjero.rt_car)
            and self.granjero.codigo == 31
            and self.nivel_actual == 2
        ):
            self.flores.continuar()
            self.anim_fondo.add(self.siembra)
            self.anim_fondo.remove(self.flecha)
    ...
    def det_msj_n2(self):
        ...
        if self.marcador.id == "sem_car":
            ...
            if self.granjero.codigo in range(0, 8) or self.granjero.codigo in range(
                16, 24
            ):
                return 0
    ...
    def actualizar_pj(self):
        ...
        if self.nivel_actual == 2:
            ...
            if self.granjero.codigo in range(1, 32, 2):
                self.grupo_marcadores.add(self.sem_rec)
            if self.granjero.codigo not in [
                0,
                1,
                4,
                5,
                ...
                self.grupo_marcadores.add(self.rega_rec)
            if self.granjero.codigo in range(8, 15) or self.granjero.codigo in range(
                24, 31
            ):
    ...
    def detectar_colision(self):
        ...
        if self.granjero.rt_car.collidelist(self.grupo_objetos.sprites()) != -1:
            self.sprite = self.grupo_objetos.sprites()[
                self.granjero.rt_car.collidelist(self.grupo_objetos.sprites())
            ]
            self.choque = True
        else:
            self.choque = False
    ...
    def colision_marcador(self):
        ...
        if self.granjero.rect.collidelist(self.grupo_marcadores.sprites()) != -1:
            self.marcador = self.grupo_marcadores.sprites()[
                self.granjero.rect.collidelist(self.grupo_marcadores.sprites())
            ]
            self.marker = True
        else:
            self.marker = False

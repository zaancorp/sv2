...
class actividad(pantalla.Pantalla):
    def __init__(self, parent):
        ...
        self.popupbien = PopUp(
            self.parent, ("prueba",), "Aceptar", self.img1, self.grupo_popup
        )
        self.popupnobien = PopUp(
            self.parent, ("prueba",), "Aceptar", self.img1, self.grupo_popup
        )
        self.instruccion1 = "    Pulsa la tecla F1 para ver las instrucciones. "
        self.texto = (
            ...
            "Pulsa la tecla F1 para iniciar la actividad. "
        )
        self.imagen_aplaudiendo = pygame.image.load(
            self.pops + "aplaudiendo.png"
        ).convert_alpha()
        self.imagen_pensando = pygame.image.load(
            self.pops + "pensando.png"
        ).convert_alpha()
        self.imagen_vacio = pygame.image.load(
            self.varios + "cuadro-texto-popup.png"
        ).convert_alpha()
        self.germinador = pygame.image.load(
            self.pops + "germinador.png"
        ).convert_alpha()
        self.dic = {
            "ENTER": self.img1,
            "TABULACIÓN": img2,
        ...
        self.popup_instruccion_fija = PopUp(
            self.parent,
            self.instruccion1,
            "Aceptar",
            self.dic,
        ...
        self.popupvacio = PopUp(
            self.parent, (self.instruccion1,), "Aceptar", self.img1, self.grupo_popup
        )
        self.popupinstruccion = PopUp(
            self.parent,
            self.texto_ayuda,
            "Aceptar",
            self.dic,
    ...
    def nivel1(self):
        ...
        self.rectangulo_texto.image = pygame.image.load(
            self.varios + "cuadro-texto.png"
        ).convert()
        self.rectangulo_texto.rect = self.rectangulo_texto.image.get_rect()
        self.grupo_sprite.add(
            self.boton_ayuda, self.boton_x, self.rectangulo_texto, self.boton_mesa
        )
        animacion1 = animacion(
            "anim1", self.varios + "reloj.png", 5, 1, 446, 38, None, True, 100
        )
        animacion2 = animacion(
            "anim2", self.varios + "obreros.png", 6, 1, -76, -39, None, True, 6
        )
        animacion3 = animacion(
            "anim3", self.varios + "caja.png", 8, 1, 520, 355, None, True, 36
        )
        ...
        self.boton.set_center(
            666 + (self.menu3.get_rect().w / 2),
            self.popup_instruccion_fija.tam + texto.ancho_final + 80,
        )
        self.texto_bien = "    ¡Muy bien! Sabías que... un trozo de tallo verde que sea introducido en la tierra para multiplicar una planta tiene por nombre esqueje. "
    ...
    def nivel2(self):
        ...
        animacion1 = animacion(
            "anim0_1", self.varios + "animacion4.png", 10, 1, -40, 0, None, True, 25
        )
        self.grupo_anim.add(animacion1)
        self.fondo = pygame.image.load(self.varios + "fondoact4.png").convert()
        self.pregunta = "    En una cesta hay 60 sobres de semillas, de ellos 1/5 son de pimentón, 1/2 son de girasol y el resto de perejil. ¿Cuántos sobres son de semillas de perejil? "
        texto = texto2(
            700,
            self.popup_instruccion_fija.tam,
            self.pregunta,
        ...
        self.texto_bien = "    ¡Excelente! Para evitar la deforestación y contribuir con el cuidado del ambiente, cuando vayas de visita a los parques recoge los desechos que te hayan quedado durante tu visita. "
        self.texto_mal = (
        ...
        self.texto_vacio = "    Para continuar deberás contestar correctamente la pregunta. Si la casilla queda vacía no podrás avanzar al siguiente problema. "
        self.texto_ayuda = "    Al construir la ecuación utiliza los 60 sobres como la unidad. Luego de hallar el valor en fracciones transformala a números naturales. "
    ...
    def nivel3(self):
        ...
        animacion1 = animacion(
            "anim_1", self.varios + "animacion5.png", 6, 1, -30, 44, None, True, 25
        )
        self.grupo_anim.add(animacion1)
        self.pregunta = "  Una distribuidora de flores recibió 12.831 bolívares por concepto de las ventas durante el mes de marzo. Si vendieron 987 flores, ¿Cuál es el costo de cada flor? "
        ...
        self.texto_bien = "   ¡Muy bien! ¿Has hecho alguna vez un germinador con semillas de caraota? Con la ayuda de tu maestra o maestro investiga los pasos a seguir para que una semilla se reproduzca y se logre obtener una nueva planta. "
        self.texto_mal = "    Recuerda: debes separar en partes iguales para obtener el valor de cada flor. "
        self.texto_vacio = "    Para continuar deberás contestar correctamente la pregunta. Si la casilla queda vacía no podrás avanzar al siguiente problema. "
        self.texto_ayuda = "    Existe una operación básica que te permite repartir equitativamente una cantidad entre un cierto número. Es el proceso contrario a la multiplicación. "
        self.spserver.processtext(
            "problema número 3:"
            + self.pregunta
            + "escribe tu respuesta y utiliza la tecla ENTER para confirmar",
            ...
            if tipo_mensaje == "bien":
                ...
                if self.nivel == 3:
                    ...
                    self.popupbien = PopUp(
                        ...
                        (self.texto_bien,),
                        "Aceptar",
                        (self.germinador, self.imagen_aplaudiendo),
                        self.grupo_popup,
                    )
    ...
    def handleEvents(self, eventos):
        ...
        for event in eventos:
            ...
            if (
                not self.popupbien.activo
                and not self.popupnobien.activo
                and not self.popupvacio.activo
            ):
            ...
            if self.click(event):
                ...
                if (
                    not self.popupayuda.activo
                    and not self.popupbien.activo
                    and not self.popupnobien.activo
                ):

                    if not self.popupinstruccion.activo:
                        if self.rectangulo_texto.rect.collidepoint(
            ...
            if event.type == pygame.KEYDOWN:
                if teclasPulsadas[pygame.K_TAB]:
                    if (
                        not self.popupinstruccion.activo
                        and not self.popupayuda.activo
                        and not self.popupbien.activo
                        and not self.popupnobien.activo
                    ):
                        if self.teclado == 0:
